...
export class GamePhysics {
    ...
    update(sceneGraph : SceneGraph) : void {
        ...
        let main_character_radius = Math.pow(Math.pow(main_character.getSpriteType().getSpriteWidth()/2, 2) + Math.pow(main_character.getSpriteType().getSpriteHeight()/2, 2), 0.5);
        for(let sprite of <Array<AnimatedSprite>>sceneGraph.scope()) {
            if(main_character != null && sprite.getTypeName() != "MAIN_BUG"){
                ...
                let sprite_radius = Math.pow(Math.pow(sprite.getSpriteType().getSpriteWidth()/2, 2) + Math.pow(sprite.getSpriteType().getSpriteHeight()/2, 2), 0.5);
